...
2026/07/23 租用 32GB vGPU 服务器（autodl），迁移项目并继续实验。

**Exp 5a：RE-ARC 从头训练（续训 40 epoch）**

| 设置 | 值 |
|------|-----|
| 硬件 | vGPU 32GB, 12C, 62GB RAM |
| batch_size | 1→2→8→2（OOM 调整，最终 batch=2） |
| LR | 5e-4，cosine decay 100 epoch |
| checkpoint | varc_rearc.pt（从本地 epoch 13 续训） |
| rearc_prob | 1.0 |
| 训练结果 | 达 epoch 40，train_loss=1.15→?，val_loss=0.98→? |
| 评估 (50) | **0%** |

**Exp 5b：Base checkpoint 微调（RE-ARC, prob=0.5）**

从 `varc_exp3_base.pt`（epoch 76, 3.5%）出发，50% 概率 RE-ARC 增强，微调 50 epoch。

| 指标 | Base (76 epoch) | RE-ARC FT (50 epoch) |
|------|----------------|---------------------|
| 准确率 (50) | 2% | **0%** |
| 准确率 (50, 8-view) | — | **0%** |
| 准确率 (10, TTT 100) | — | **0%** |

**结论：RE-ARC 增强在 400 任务小数据集上破坏了 task embedding，增强带来的干扰 > 收益。**

### VARC Exp 6（多视角推理 + TTT 服务器测试）

在 32GB 服务器上测试论文的核心技巧：

| 方法 | 准确率 (50) | 说明 |
|------|-------------|------|
| 单视角 | 0% | RE-ARC FT 模型 |
| 8 视角 flip/rot 投票 | 0% | 增强混淆 |
| TTT 100 步 (10 task) | 0% | 模型太弱无法适配 |

**最终判定：VARC 路线在当前条件下不收敛。** 核心瓶颈——数据量太小（400 tasks, ~1600 pairs），RE-ARC 增强后样本空间扩大但 task 身份信息被稀释。论文使用 10k 种子任务 × 40 倍增强 = 400k/epoch，我们差距 250 倍。
...
| VARC RE-ARC FT (Exp 5) | 50 | **0%** | 从 base 微调, rearc_prob=0.5, 50 epoch |
| VARC + 多视角 + TTT (Exp 6) | 50 | **0%** | 8-view vote + TTT 100 steps 均无效 |
...
7. **VARC 在数据不足时无法收敛**：400 任务不足以训练 VARC（论文需 400k 样本/epoch），RE-ARC 增强在大模型 + 大数据的背景下才有效
8. **TTT 和 Multi-view 依赖强基模型**：多视角投票和 TTT 不能弥补基模型弱的问题，必须先把模型训到一定正确率才有意义

### 待完成 / 已尝试

- ✅ **VARC + RE-ARC**：实现完成并上服务器完整训练，结论为 400 任务不足以支撑 VARC 训练
- ✅ **多视角推理**：8 视角 flip/rot 投票，基模型太弱无提升
- ✅ **TTT**：服务器 32GB 可跑，基模型太弱无提升
- ❌ **VARC 路线终止**：数据量不足，模型无法收敛到有意义的结果
- 📝 **案例分析**：DeepSeek vs VARC 对比可写，但 VARC 无正样本对比意义不大
- 📝 **课程设计论文**：以 DeepSeek 46.7% 为主体，VARC 作为尝试方向
