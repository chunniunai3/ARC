...
from src.varc import VARCConfig, VARCModel, ARCDataset, load_checkpoint, BG_CLASS
import math
...
for epoch in range(61, 101):
    cosine_progress = (completed_cosine + epoch - 61) / T_max
    lr = 3e-4 * 0.5 * (1.0 + math.cos(math.pi * cosine_progress))
    ...
    print(f"  Epoch {epoch:3d}/100  lr={lr:.2e}  train_loss={train_loss:.4f}  val_loss={val_loss:.4f}")
